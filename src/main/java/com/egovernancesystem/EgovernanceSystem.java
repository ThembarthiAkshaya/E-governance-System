...
public class EgovernanceSystem 
{
	public static void main(String[] args)
	{
		...
		switch (role) {
		case 1:
			isLoggedIn = loginCitizen(sc);
			if (isLoggedIn) {
				handleCitizenOperations(sc);
			}
			break;
		case 2:
			isLoggedIn = loginDepartmentRepresentative(sc);
			if (isLoggedIn) {
				handleDepartmentOperations(sc);
			}
			break;
		case 3:
			isLoggedIn = loginGovernmentEmployee(sc);
			if (isLoggedIn) {
				handleEmployeeAndServiceManagementOperations(sc);
			}
			break;
		default:
	...
	private static boolean loginCitizen(Scanner sc) {
		...
		{
			System.out.println("Welcome, " + citizenId + "!");
			return true;
		} 
		else
		{
			System.out.println("Citizen ID not found. Would you like to create a new account? (yes/no)");
			String response = sc.nextLine();
			if (response.equalsIgnoreCase("yes"))
			{
				CitizenController citizenController = new CitizenController();
				citizenController.insertCitizenFromInput(); // Call method to create a new citizen account
				return true; // Assume account creation is successful 
			}
		}
		return false; // Login failed
	}

	private static boolean isCitizenIdExists(String citizenId) {
		...
		try (Session session = sessionFactory.openSession()) {
			...
			// HQL query to check if the citizen ID exists
			String hql = "SELECT COUNT(c) FROM Citizen c WHERE c.citizenId = :citizenId"; // Adjust the property name as necessary
			Query<Long> query = session.createQuery(hql, Long.class);
			query.setParameter("citizenId", citizenId);

			Long count = query.uniqueResult(); // Get the unique result
			exists = (count != null && count > 0); // If count is greater than 0, the ID exists
	...
	private static boolean loginDepartmentRepresentative(Scanner sc) {
		System.out.print("Enter Department ID: ");
		String departmentId = sc.nextLine();

		// Check if the department ID exists in the database
		if (isDepartmentIdExists(departmentId)) {
			System.out.println("Welcome to the Department with ID: " + departmentId + "!");
			return true;
		} else {
	...
	private static boolean isDepartmentIdExists(String departmentId)
	{
		boolean exists = false;
		SessionFactory sessionFactory = HibernateUtils.getSessionFactory();
		Transaction transaction = null;
		...
		try (Session session = sessionFactory.openSession())
		{
			...
			// HQL query to check if the department ID exists
			String hql = "SELECT COUNT(d) FROM Department d WHERE d.departmentId = :departmentId"; // Adjust the property name as necessary
			Query<Long> query = session.createQuery(hql, Long.class);
	...
	private static boolean loginGovernmentEmployee(Scanner sc)
	{
		...
		{
			System.out.println("Welcome, Employee ID: " + employeeId + "!");
			return true;
		} 
		else
		{
	...
	private static boolean isEmployeeIdExists(String employeeId)
	{
		...
		try (Session session = sessionFactory.openSession()) {
			...
			// HQL query to check if the employee ID exists
			String hql = "SELECT COUNT(e) FROM Employee e WHERE e.employeeId = :employeeId"; // Adjust the property name as necessary
			Query<Long> query = session.createQuery(hql, Long.class);
		...
		try {
			while(true)
			{
				...
				case 3:
					...
					break;
				case 4:
					employeecontroller.deleteEmployeeFromInput();
					break;
				case 0:
					System.out.println("Exiting to Department menu...");
					return;
	...
	public static void manageEmployeeApprovals(Scanner sc)
	{
		try
		{
			while(true) 
			{
				...
				System.out.println("1.View Employee Approvals ");
				System.out.println("2.Assign Approval to Employee ");
				System.out.println("3. Remove Approval from Employee");
				System.out.println("0.Exit");
				System.out.print("Select an option: ");
				int option = sc.nextInt();
				...
				case 1:
					...
					break;
				case 2:
					approvalcontroller.insertApprovalFromInput();
					break;
				case 3:
					approvalcontroller.deleteApprovalsByEmployeeId();
					break;
				case 0:
			...
			System.out.println("Error while managing employee services: " + e.getMessage());
			e.printStackTrace();
		}
	}
